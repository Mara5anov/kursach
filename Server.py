...
work_on_server = config['SERVER_SETTINGS']['isWorkingOnServer']
server_ip = str(config['SERVER_SETTINGS']['server_ip'])
port = int(config['SERVER_SETTINGS']['port'])
...
microbes = [Food(random.randint(0, WIDTH_ROOM),
                 random.randint(0, HEIGHT_ROOM),
                 MICROBE_SIZE,
                 str(random.randint(0, 4)))
...
while server_works:
    ...
    if tick == 200:
        ...
        new_microbes = [Food(random.randint(0, WIDTH_ROOM),
                             random.randint(0, HEIGHT_ROOM),
                             MICROBE_SIZE,
                             str(random.randint(0, 4)))
                        for i in range(FOOD_QUANTITY - len(microbes))
                        ]
    ...
    # отправляем данные каждому игроку
    for i in range(len(players)):
